...
export async function POST() {
  ...
  try {
    ...
    // ── プロンプト構築 ────────────────────────────────────────
    const dataContext = `
## ジブキャリ管理ダッシュボード データ（${now.toLocaleDateString("ja-JP")} 時点）
...
`;
    ...
    const systemPrompt = `あなたはSaaSプロダクトの成長を専門とするプロダクトアナリストです。
ジブキャリ（日本語の履歴書作成Webアプリ）の管理ダッシュボードデータを分析し、
日本語で簡潔かつ実用的なレポートを作成してください。
        ...
        try {
          const response = await anthropic.messages.stream({
            ...
            messages: [
              {
                role: "user",
                content: `以下のデータを分析してレポートを作成してください：\n${dataContext}`,
              },
            ],
          });
